...
3. Run the test script to verify:
   // turbo
   `bash tests/manual/test-<name>.sh`
4. Create `wiki/<name>.sh.md` with usage documentation.
   - Include: module description, bash requirement, dependencies, include guard, all functions with signatures and examples.
...
8. Update `AGENTS.md` architecture quick reference if the module fits in the layer diagram.
9. Sync `.windsurfrules` and `.clinerules` if `AGENTS.md` changed.
10. Update `README.md` project structure and docs sections.
11. Run all manual tests to check for regressions:
    // turbo
