...
- **`deps.sh`** is standalone. It provides `deps::need <tool>...` to assert external binaries are on `$PATH`; missing tools are listed in a single stderr line prefixed with `${args_program_name:-${0##*/}}:` and `exit 1` is triggered. Uses plain `echo` so it is safe to call before any ANSI module loads. The companion `deps::have <tool>...` is a silent predicate — returns 0 when every tool is present, 1 otherwise — for soft fallbacks.
...
3. Optionally set `args_program_default_command` to a registered command name.
4. Call `args::parse "$@"` to invoke `getopt` and populate `args_options` (associative array) and `args_command`.
5. Registered `args::on_options` hooks run.
6. Immediate options (`-h`, `-v`, `--bash-completion`) trigger handler functions (`args::option::help`, `args::option::version`, `args::option::completion`) and exit.
7. If no command matched and `args_program_default_command` is set, it is applied.
8. Registered `args::on_parse` hooks run after successful completion.
9. Remaining arguments are in `args_cleaned` (array).
