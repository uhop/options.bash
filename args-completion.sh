...
args::completion::generate() {
  ...
  echo "# Generated by args-completion.sh (options.bash)"
  echo ""
  echo "${func_name}() {"
  ...
  local fzf_func_name="_fzf_complete_${name//[^a-zA-Z0-9_]/_}"
  local all_items=("${options[@]}")
  [[ ${#commands[@]} -gt 0 ]] && all_items+=("${commands[@]}")

  echo ''
  echo "${fzf_func_name}() {"
  echo "  _fzf_complete --height=40% --reverse -- \"\$@\" < <("
  echo "    printf '%s\\n' ${all_items[*]}"
  echo '  )'
  echo '}'
}
...
args::completion::register() {
  ...
  if [[ -f "$comp_file" && "$comp_file" -nt "$script_path" ]]; then
    return 0
  fi
